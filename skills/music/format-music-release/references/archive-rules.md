...
Title, Artist, Album Artist, Album, Year, Track, Disc Number, Compilation, and Artwork are required for a fully formatted release. Catalog and Label are also required when the release has them. BPM, Genre and its matching Style value, Comment, and Composer are conditional: preserve or set them only from defensible evidence. Never write `Unknown`, `N/A`, an empty string, or inferred credits merely to populate a field.
